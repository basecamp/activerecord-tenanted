...
#### Copying or moving between the untenanted database and the new tenants

Add the previous database as a replica shard:

``` patch
diff --git a/config/database.yml b/config/database.yml
index d0aa1fb1..c3e29a17 100644
...
+  primary_original:
+    <<: *default
+    database: storage/production-backup-202503051235.sqlite3
+    replica: true
```

And configure your connection class to treat it as a readonly shard:

``` ruby
ApplicationRecord.connects_to(shards: { untenanted: { reading: :primary_original } })
```

Then, to connect to the old database:

``` ruby
ApplicationRecord.connected_to(role: :reading, shard: :untenanted) { ... }
```

And connect to the new tenants as normal:

``` ruby
ApplicationRecord.while_tenanted(tenant) { ... }
```
